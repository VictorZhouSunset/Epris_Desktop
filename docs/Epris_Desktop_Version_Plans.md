...
<!-- - 但如果你要“登录后才能用”，需要后端做身份与权限校验（至少做到：未登录不能进入主功能）。 （暂时不用）-->
...
- Windows 可安装运行。
- 预览窗口 + 单 prompt 输入框 + 版本管理 + 多项目管理 + 语音转文字。
...
- Desktop App CLI后台引入 superpowers 的“先写文件级计划再执行”作为安全刹车。

import { Send, RefreshCw, Plus, Download, FolderOpen, Clock, Save, Settings as SettingsIcon } from 'lucide-react';
...
function App() {
  ...
  const workspacePath = workspaceStatus === 'ready' ? workspace?.path : undefined;
  
  const { status: previewStatus, port } = usePreviewServer(workspacePath);
  const { status: openCodeStatus } = useOpenCode(workspacePath); // TODO: Make this conditional on provider? backend handles lazy start
  ...
  const { status: promptStatus, progress: promptProgress, currentStep: promptCurrentStep, error: promptError, lastResponse, sendPrompt, clearSession } = usePrompt(
    workspacePath,
    {
      onSuccess: () => {
        // Reload preview after AI modification
        reloadPreview();
      },
    },
    provider,
  ...
  const { status: exportStatus, progress: exportProgress, outputPath, error: exportError, exportVideo, openOutputFolder } = useExport(workspacePath);
  ...
  useEffect(() => {
    ...
    const checkEnv = async () => {
       try {
          ...
          const ready = status.node_valid && status.pnpm_valid && status.provider_cli_valid && status.workspace_deps_valid && status.skills_valid;
          setIsEnvReady(ready);
          if (!ready) {
             setShowWizard(true);
          }
  ...
  const overallStatus = useMemo(() => {
    if (workspaceStatus === 'loading') return { text: 'Initializing...', color: 'bg-yellow-500' };
    if (workspaceStatus === 'error') return { text: 'Workspace Error', color: 'bg-red-500' };
    ...
    // Only warn about AI if waiting for it when needed, but OpenCode is usually always running or lazy
  ...
  }, [workspaceStatus, previewStatus, openCodeStatus, promptStatus, exportStatus, lastResponse]);
  
  const isReady = previewStatus === 'running' && openCodeStatus === 'running' && isEnvReady;
        ...
        <div className="flex items-center gap-2">
          ...
          {exportStatus === 'done' && outputPath ? (
            ...
            >
              ...
              <span className="hidden sm:inline">{exportStatus === 'exporting' ? `Exporting ${Math.round(exportProgress)}%` : 'Export'}</span>
            </button>
          )}
          ...
          {isProcessing && (
            <div className="absolute inset-0 bg-slate-900/80 flex items-center justify-center z-30">
              <div className="text-center w-full max-w-md px-10">
                ...
                <div className="text-xl font-semibold text-white mb-2">{promptCurrentStep || 'Generating Animation...'}</div>
                ...
                <div className="text-xs text-slate-500 uppercase tracking-widest font-bold tracking-widest">{Math.round(promptProgress)}% Complete</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="w-full max-w-4xl mx-auto flex flex-col gap-3">
          <div className="relative group flex gap-3 items-center">
             ...
             <div className="relative">
               ...
               >
                  <span className={`w-2 h-2 rounded-full ${provider === 'gemini' ? 'bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,0.5)]' : 'bg-green-400 shadow-[0_0_8px_rgba(74,222,128,0.5)]'}`}></span>
                  <div className="flex flex-col items-start leading-none gap-1">
                     <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{provider === 'gemini' ? 'Gemini' : 'OpenCode'}</span>
                     <span className="text-xs font-bold text-slate-300 max-w-[100px] truncate">{model.split('/').pop()}</span>
                  </div>
               </button>
                   ...
                   <div className="absolute bottom-full mb-2 left-0 w-64 bg-slate-900 border border-slate-700 rounded-xl shadow-xl z-20 overflow-hidden py-1">
                      ...
                      {MODELS[provider as keyof typeof MODELS]?.map(m => (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 text-sm hover:bg-slate-800 transition-colors flex items-center justify-between ${model === m.id ? 'text-indigo-400 bg-slate-800/50' : 'text-slate-300'}`}
                        >
                      ...
                      <div className="border-t border-slate-800 mt-1 pt-1">
                         <button 
                            onClick={() => { setShowSettings(true); setShowModelMenu(false); }}
          ...
          <p className="px-4 text-sm text-slate-500">
            {isReady 
              ? "Press Enter or click send to generate code with AI."
              : "Waiting for preview server and OpenCode to start..."}
          </p>
        </div>
      </main>
      ...
      <footer className="px-6 py-3 border-t border-slate-800 bg-slate-900 flex items-center justify-between text-xs font-medium text-slate-500">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1.5">
            <span className={`w-2 h-2 rounded-full ${overallStatus.color} shadow-[0_0_8px_rgba(16,185,129,0.5)]`}></span>
            {overallStatus.text}
          </span>
          ...
          <span className="truncate max-w-xs flex items-center gap-2">
            ...
            {hasUnsavedChanges && (
              <span className="px-1.5 py-0.5 bg-amber-500/10 text-amber-500 rounded text-[10px] font-bold border border-amber-500/20">MODIFIED</span>
            )}
          </span>
        </div>
        <div className="flex items-center gap-4">
          {lastResponse?.gate_result && (
            <span className={`px-2 py-0.5 rounded-full ${lastResponse.gate_result.passed ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}>
          ...
          {lastResponse?.snapshot_id && (
            <span className="text-slate-600">
              SN: {lastResponse.snapshot_id.substring(0, 8)}...
            </span>
          )}
